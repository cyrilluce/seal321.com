...
import { observable, computed, action, reaction, autorun, when } from "mobx";
...
export class GSimulate extends Base {
  ...
  /** 是否是高級料理 */
  @computed
  get advanceCook(): boolean {
    const book = this.book.data;
    const craft = this.craft.data;
    return (
      !!book &&
      !!craft &&
      book.type === ItemType.FOOD &&
      craft.type === CraftType.NORMAL
    );
  }
  /** 书本错误 */
  @computed
  get bookInvalid(): string {
    if (!this.book.data) {
      return "请放置制作书";
    }
  }
  ...
  get invalid(): string {
    return (
      this.bookInvalid ||
      this.targetInvalid ||
      this.assistInvalids.filter(err => err)[0]
    );
  }
  @computed
  get loading(): boolean {
    return (
      (this.book && this.book.loading) ||
  ...
  /** 成功率限定系数 */
  @computed
  get limitRate(): number {
    // 只有装备制作才有最高成功率限制
    return this.craft.isGTSC ? this.maxRate : 1;
  }
  /** 基础成功率 */
  @computed
  get basePercentage(): number {
  ...
  get percentage(): number {
    ...
    return (
      this.limitRate * (this.basePercentage + 100 * this.hasPt / this.needPt) ||
      0
    );
  }
  /** 匠师技能是否有效 */
  @computed
  get skillWork(): boolean {
    const { craft, target } = this;
  ...
  tryTargetItem(item: Item): string {
    ...
    } else if (item.id !== craft.needitem) {
      // 如果是其它制作书，直接限定了材料
      return "制作书已限定主要材料";
    }
  }
  /** 设置被强化装备 */
  @action
  setTargetItem(item?: Item): string {
    if (!item) {
      this.target.setId(0);
      return;
    }
  ...
  /** 设置辅助ID */
  @action
  setAssistItemId(index: number, itemId: number) {
    this.setAssistItem(index, null, itemId);
  }
  /** 设置辅助 */
  @action
  setAssistItem(index: number, item?: Item, itemId?: number): string {
    ...
    when(
      "item loaded",
      () => itemModel.data && !itemModel.loading,
      () => {
